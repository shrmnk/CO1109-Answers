...
class Tictac3 {
    ...
    static int[] getWinningCoordinates(int[][] markings, int player) {
        int[] ret = { -1, -1 };
        // Check horizontal win
        for(int row = 0; row < 3; row++) {
            // 0,1 filled and 2 blank
            if(markings[row][0] == player && markings[row][0] == markings[row][1] && markings[row][2] == 0) return new int[] {row, 2};
            // 0,2 filled and 1 blank
            else if(markings[row][0] == player && markings[row][0] == markings[row][2] && markings[row][1] == 0) return new int[] {row, 1};
            // 1,2 filled and 0 blank
            else if(markings[row][1] == player && markings[row][1] == markings[row][2] && markings[row][0] == 0) return new int[] {row, 0};
        }

        // Check vertical win
        for(int column = 0; column < 3; column++) {
            // 0,1 filled and 2 blank
            if(markings[0][column] == player && markings[0][column] == markings[1][column] && markings[2][column] == 0) return new int[] {2, column};
            // 0,2 filled and 1 blank
            else if(markings[0][column] == player && markings[0][column] == markings[2][column] && markings[1][column] == 0) return new int[] {1, column};
            // 1,2 filled and 0 blank
            else if(markings[1][column] == player && markings[1][column] == markings[2][column] && markings[0][column] == 0) return new int[] {0, column};
        }
    ...
    public static void main(String[] args) {
        ...
        while(winner == 0 && !isBoardFilled(currentMarkings)) {
            ...
            if(currentPlayer == AI_PLAYER) {
                int[] coordinatesAI = new int[2];
                // Best-case: Check for winning move
                // Mid-case: Fill a line with a possible winning move
                // Worst-case: Generate random move
                coordinatesAI = getWinningCoordinates(currentMarkings, AI_PLAYER);
                if(coordinatesAI[0] == -1 || coordinatesAI[1] == -1)
                    coordinatesAI = generateRandomMove(currentMarkings);
